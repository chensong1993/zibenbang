...
public class PDFView extends SurfaceView {
    ...
    void showPage(int pageNb) {
        ...
        if (onPageChangeListener != null&&currentPage>=0&&getPageCount()>=0) {
            try {
           ...
           // onPageChangeListener.onPageChanged(0, 0);
            //onPageChangeListener.onPageChanged(1, 1);
        }
    }
    ...
    public void loadComplete(DecodeService decodeService) {
        ...
        // Notify the listener
        jumpTo(defaultPage);
        if (onLoadCompleteListener != null&&documentPageCount!=0) {
