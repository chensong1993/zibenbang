...
public class MACDDrawing implements IDrawing {
    ...
    public void onInit(RectF contentRect, AbstractRender render) {
        ...
        if (macdPaint == null) {
            ...
            macdPaint.setStrokeWidth(chartConfigs.getMacdLineSize());
        }
        macdPaint.setColor(chartConfigs.getIncreasingColor());

        deaPaint.setStrokeWidth(chartConfigs.getMacdLineSize());
        difPaint.setStrokeWidth(chartConfigs.getMacdLineSize());

        deaPaint.setColor(chartConfigs.getDeaLineColor());
        difPaint.setColor(chartConfigs.getDiffLineColor());
    ...
    public void onComputeOver(Canvas canvas, int minIndex, int maxIndex, float minY, float maxY) {
        ...
        for (int i = minIndex; i < maxIndex; i++) {
            ...
            if (macdBuffer[3] <= gridBuffer[1]) {
                ...
                macdPaint.setColor(chartConfigs.getDecreasingColor());
            }
