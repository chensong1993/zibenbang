...
public class EntrySet {
    ...
    public void insertAt(Entry entryWithTime) {
        int updatedIndex = 242;

        for (int i = 3; i < entries.size(); i++) {
            Entry entry = entries.get(i);
            if (entry.getXLabel().equals(entryWithTime.getXLabel())) {
                entries.remove(i);
                entryWithTime.setUpdated(true);
                entries.add(i, entryWithTime);
                updatedIndex = i;
                Log.d("StockChart: Entries", "Adding item at " + i + " , with time " + entryWithTime.getXLabel());
            }
        }

        for (int i = updatedIndex + 1; i < entries.size(); i++) {
            Entry entry = entries.get(i);
            if (entry.isUpdated())
                break;
            Entry afterEntry = new Entry(entryWithTime.getOpen(), entryWithTime.getHigh(), entryWithTime.getLow(), entryWithTime.getClose(), 0, entryWithTime.getAverage(), entries.get(i).getXLabel());
            entries.remove(i);
            entries.add(i, afterEntry);
        }

        SimpleDateFormat parser = new SimpleDateFormat("HH:mm");
        try {
            Date lastUpdated = parser.parse(entryWithTime.getXLabel());
            Date startAM = parser.parse("9:30");
            int updatingIndex1 = 0;
            if (lastUpdated.before(startAM)) {
                Entry afterEntry = new Entry(entryWithTime.getOpen(), entryWithTime.getHigh(), entryWithTime.getLow(), entryWithTime.getClose(), entryWithTime.getVolume(), entryWithTime.getAverage(), entries.get(updatingIndex1).getXLabel());
                entries.remove(updatingIndex1);
                entries.add(updatingIndex1, afterEntry);
            }

            Date stopPM = parser.parse("15:00");
            int updatingIndex2 = 242;
            if (lastUpdated.after(stopPM)) {
                Entry afterEntry = new Entry(entryWithTime.getOpen(), entryWithTime.getHigh(), entryWithTime.getLow(), entryWithTime.getClose(), entryWithTime.getVolume(), entryWithTime.getAverage(), entries.get(updatingIndex2).getXLabel());
                entries.remove(updatingIndex2);
                entries.add(updatingIndex2, afterEntry);
            }
        } catch (Exception e) {
            Log.e("EntrySet: Insert", "Got error during insert last entry");
            e.printStackTrace();
        }
     ...
     * @param end
     */
    public void computeTimeLineMinMax(int start, int end, List<StockIndex> stockIndexList) {
        int endValue;
        ...
        for (int i = start; i <= endValue; i++) {
            ...
            if (entry.getClose() < minY) {
                minY = entry.getClose();
                minYIndex = i;
                minDelta = getPreClose() - minY;
            }

            if (entry.getClose() > maxY) {
                maxY = entry.getClose();
                maxYIndex = i;
                maxDelta = maxY - getPreClose();
            }
        ...
        if (minDelta > maxDelta) {
            minY = getPreClose() - minDelta;
            maxY = getPreClose() + minDelta;
        } else {
            minY = getPreClose() - maxDelta;
            maxY = getPreClose() + maxDelta;
        }

        if (minY == maxY) {
            minY = minY - minY * 0.02F;
            maxY = maxY + maxY * 0.02F;
        }
