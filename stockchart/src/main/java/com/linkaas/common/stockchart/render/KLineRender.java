...
import com.linkaas.common.stockchart.drawing.CandleDrawing;
...
import com.linkaas.common.stockchart.drawing.KLineGridAxisDrawing;
import com.linkaas.common.stockchart.drawing.MADrawing;
...
//        kLineDrawingList.add(maDrawing);
    ...
    public void onViewRect(RectF viewRect) {
        ...
        for (StockIndex stockIndex : stockIndexList) {
            if (stockIndex.isEnable()) {
                ...
                float top = kLineRect.bottom + chartConfigs.getXLabelViewHeight() + calculateHeight - stockIndex.getHeight();
                float bottom = kLineRect.bottom + chartConfigs.getXLabelViewHeight() + calculateHeight;
    ...
    private void computeVisibleCount() {
        zoomInTimes = Math.abs(chartConfigs.getZoomInTimes() == 0 ? 3 : chartConfigs.getZoomInTimes());
        zoomOutTimes = Math.abs(chartConfigs.getZoomOutTimes() == 0 ? 3 : chartConfigs.getZoomOutTimes());
        ...
        if (currentVisibleCount == -1) {
            currentVisibleCount = chartConfigs.getPortraitDefaultVisibleCount();
