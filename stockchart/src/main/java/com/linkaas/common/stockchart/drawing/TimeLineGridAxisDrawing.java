...
public class TimeLineGridAxisDrawing implements IDrawing {
    ...
    public void onInit(RectF contentRect, AbstractRender render) {
        ...
        xLabelPaint.setTextSize(chartConfigs.getXLabelSize());
        xLabelPaint.setColor(chartConfigs.getXLabelColor());
        ...
        gridPaint.setStrokeWidth(chartConfigs.getGridSize());
        gridPaint.setColor(chartConfigs.getGridColor());
      //  gridPaint.setPathEffect(new DashPathEffect(new float[]{5, 10, 15, 20}, 0));
    ...
    public void onComputeOver(Canvas canvas, int minIndex, int maxIndex, float minY, float maxY) {
        ...
        //float lineTop = chartRect.top + lineHeight;
        Path mPath;
        mPath = new Path();
        for (int i = 0 ; i < 5 ; i++) {
            float lineTop = chartRect.top + i * lineHeight;
            mPath.moveTo(5, lineTop);
            mPath.quadTo(chartRect.left, lineTop, chartRect.right, lineTop);
        }
            ...
            if (entrySetSize > 0) {
                if (entrySet.getEntryList().size() > i) {
                    canvas.drawText(
                            ...
                            lineLeft,
                            chartRect.bottom + render.getChartConfigs().getXLabelSize(),
                            xLabelPaint);
                }
            }
        }
    ...
    public void onDrawOver(Canvas canvas) {
        ...
        for (int i = 0 ; i < 5 ; i++) {
            ...
            if (i == 2) {
                yLabelPaint.setColor(chartConfigs.getNeutralColor());
            } else if (i > 2) {
                yLabelPaint.setColor(chartConfigs.getDecreasingColor());
            } else {
                yLabelPaint.setColor(chartConfigs.getIncreasingColor());
            }
