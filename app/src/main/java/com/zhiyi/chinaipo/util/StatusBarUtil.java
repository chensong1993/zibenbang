...
public class StatusBarUtil {
    ...
    public static void enableTranslucentStatusbar(Activity activity, TextView textView, int layoutType) {
    ...
    public void setStatusMiUI(Activity statusMiUI) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Window window = statusMiUI.getWindow();
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            window.setStatusBarColor(Color.BLACK);
...
//            }
    ...
    public static int StatusBarLightMode(Activity activity, boolean black) {
        int result = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (MIUISetStatusBarLightMode(activity.getWindow(), black)) {
                result = 1;
            } else if (FlymeSetStatusBarLightMode(activity.getWindow(), black)) {
                result = 2;
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (black) {
                    activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
     ...
     *
     * @param activity       需要设置的activity
     * @param needOffsetView 需要向下偏移的 ViewI
     */
